...
if 'logged_in' in st.session_state and st.session_state['logged_in']:
    ...
    engine = create_engine('postgresql+psycopg2://postgres:Trai%401234@localhost:5432/postgres')
    ...
    def get_data():
        with engine.connect() as conn:
            ...
            return df, quarter_options, tsp_options, lsa_code_options, technology_options

    try:
        df, quarter_options, tsp_options, lsa_code_options, technology_options = get_data()

        with st.container():
            col1, col2, col3 = st.columns([1, 1, 1])

            with col1:
                quarter_selected = st.selectbox("Select Quarter", ["Quarter"] + quarter_options, index=0)
            with col2:
                tsp_selected = st.selectbox("Select TSP", ["TSP"] + tsp_options, index=0)
            with col3:
                lsa_code_selected = st.selectbox("Select LSA Code", ["LSA Code"] + lsa_code_options, index=0)

        st.sidebar.markdown('<div style="text-align: center; font-size: 1.0em; color: #007BFF;">Filter Technology</div>', unsafe_allow_html=True)
        tech_selected = st.sidebar.multiselect("", technology_options, default=technology_options, key='tech_selected')

        st.sidebar.markdown('<div style="text-align: center; font-size: 1.0em; color: #007BFF;">Days Cell Q(90) Value > 2% [Select Range]</div>', unsafe_allow_html=True)
        ...
        st.sidebar.markdown('<div style="text-align: center; font-size: 1.0em; color: #007BFF;">Cell Q(90) Value Range [Select Range]</div>', unsafe_allow_html=True)
        ...
        if show_map:
            ...
            if not filtered_df.empty:
                ...
                # Check again if filtered_df is still not empty
                if not filtered_df.empty:
                    category_order = ['Above 2 to 4', 'Above 4 to 10', 'Above 10 to 20', 'Above 20 to 50', 'Above 50']

                    # Create bar chart for Cell Q(90) counts
                    fig_bar = px.bar(
                        cell_q_90_counts,
                        x='Range',
                        y='Count',
                    ...
                    tech_counts = filtered_df.groupby('Technology')['CGI'].count().reset_index()
                    ...
                    # TSP distribution
                    tsp_counts = filtered_df.groupby('TSP')['CGI'].count().reset_index()
                    ...
                    with st.sidebar:
                        st.markdown('<div style="text-align: center; font-size: 1.0em; color: #007BFF;">Technology Distribution Analysis of Cell Q(90)> 2%(Filtered)</div>', unsafe_allow_html=True)
                        st.plotly_chart(fig_tech_pie, use_container_width=True)

                        st.markdown('<div style="text-align: center; font-size: 1.0em; color: #007BFF;">Service Provider (TSP) Distribution Analysis of Cell Q(90)> 2% (Filtered)</div>', unsafe_allow_html=True)
                    ...
                    if st.button('Show Map'):
                        with st.spinner('Processing...'):
                            ...
                            all_markers_cluster = MarkerCluster(name='Markers 📍 ', disable_clustering_at_zoom=disable_clustering_at_zoom).add_to(my_map)
                            ...
                            folium_static(my_map, width=1850, height=1000)
